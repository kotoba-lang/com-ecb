(ns ecb.main
  "Kotodama WASM entrypoint for the Ecb clean-room actor (L4) — Clojure port.
  ...
  facts (`ecb.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Indicator"   :plural "indicators"   :id-prefix "ecb_ind"
    :fields [:code :name :unit :source]          :required [:code :name]
    :coerce {}                       :refs {}}
   {:entity "Observation" :plural "observations" :id-prefix "ecb_obs"
    :fields [:indicatorId :country :period :value] :required [:country :period]
    :coerce {:value :float}          :refs {:indicatorId "Indicator"}}
   {:entity "Country"     :plural "countries"     :id-prefix "ecb_cou"
    :fields [:iso3 :name :region]                :required [:iso3 :name]
    :coerce {}                       :refs {}}
   {:entity "Series"      :plural "serieses"      :id-prefix "ecb_ser"
    :fields [:indicatorId :frequency :startPeriod] :required [:frequency :startPeriod]
    :coerce {}                       :refs {:indicatorId "Indicator"}}
   {:entity "Report"      :plural "reports"      :id-prefix "ecb_rep"
    :fields [:title :publishedAt :contentRef]    :required [:title :publishedAt]
    :coerce {}                       :refs {}}
   {:entity "ExchangeRate" :plural "exchangerates" :id-prefix "ecb_exc"
    :fields [:base :quote :rate :asOf]           :required [:base :quote]
...
(defn emit-facts
  "EAVT facts for one record: {\"ecb.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ecb-compat" :tier tier :entities entities} 200])
